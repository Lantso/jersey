...
function pageMeta(current = route()) {
  ...
  const localizedMeta = {
    ...
    fr: {
      ...
      roots: ["Découvrir ROOTS | Lantso", "L'histoire Lantso derrière ROOTS 01 Khaki et Atlas 02 White, de l'héritage marocain au monde."],
...
function mobileHeroPhoto(file, alt, width, height, loading = "lazy") {
  return {
    src: photoUrl(`/assets/photos/fallback/${file}.jpg`),
    webp: photoUrl(`/assets/photos/${file}.webp`),
    srcset: `${photoUrl(`/assets/photos/responsive/${file}-480.webp`)} 480w, ${photoUrl(`/assets/photos/responsive/${file}-800.webp`)} 800w, ${photoUrl(`/assets/photos/responsive/${file}-1080.webp`)} 1080w`,
    sizes: "100vw",
...
function visualFor(label) {
  ...
  if (key.includes("origin") || key.includes("pencil")) {
    return photo("origin-1", "campaign-visual", "Lantso origin shooting image", 1200, 1500);
  }
  ...
  return `
    <section class="gate-page">
      <figure class="gate-media" aria-hidden="true">
        <picture>
          <img src="/assets/photos/gate/foot.jpg" alt="" width="1920" height="1280" decoding="async" fetchpriority="high">
        </picture>
      </figure>
      ...
      <section class="roots-layout">
        <article class="roots-story">
          ...
          <div class="roots-image roots-image--right">
            ${visualFrame(photo("origin-1", "campaign-visual", "Lantso origin shooting portrait", 1200, 1500), "Origin image one")}
          </div>
          <div class="roots-image roots-image--left">
            ${visualFrame(photo("origin-2", "campaign-visual", "Lantso origin shooting detail", 1200, 1500), "Origin image two")}
          </div>
