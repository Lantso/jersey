...
const MIN_CHECKOUT_TTL_MS = 30 * 60 * 1000;
const DEFAULT_CHECKOUT_TTL_MS = 45 * 60 * 1000;
const CHECKOUT_TTL_MS = Math.max(MIN_CHECKOUT_TTL_MS, Number(process.env.LANTSO_CHECKOUT_TTL_MS || DEFAULT_CHECKOUT_TTL_MS));
...
export async function reserveCheckoutInventory(checkout, request = {}) {
  ...
  return mutateInventoryState((state) => {
    ...
    state.reservations[checkout.orderRef] = {
      ...
      baseCurrency: checkout.baseCurrency || "eur",
      ...
      baseSubtotal: checkout.baseSubtotal,
      subtotal: checkout.subtotal,
      baseShipping: checkout.shipping.baseAmount,
      shipping: checkout.shipping.amount,
      baseTotal: checkout.baseTotal,
...
export async function recordPaidCheckoutSession(session = {}, event = {}) {
  return mutateInventoryState((state) => {
    ...
    const stockAction = clean(session.metadata?.stock_action || reservation?.stockAction || "", 40);
    const skipsInventory = stockAction === "none";
    const items = applySessionUnitAmountOverride(
      reservation?.items?.length ? reservation.items : parseMetadataItems(session.metadata?.items),
      session,
      skipsInventory
    );
    const missingReservation = !reservation;

    if (!skipsInventory && (missingReservation || reservation.status !== "paid")) {
      markItemsSold(state, items);
    }
    ...
    baseCurrency: clean(reservation?.baseCurrency || session.metadata?.base_currency || "eur", 8),
    baseAmountSubtotal: reservation?.baseSubtotal ?? null,
    baseAmountShipping: reservation?.baseShipping ?? null,
    baseAmountTotal: reservation?.baseTotal ?? null,
      customer: {
      ...
      items,
      stockAction: stockAction || "reserve_then_sell",
      promoCode: clean(session.metadata?.promo_code || reservation?.promoCode || "", 160),
      supportTest: session.metadata?.support_test === "true" || reservation?.supportTest === true,
      orderNote: clean(session.metadata?.order_note || "", 500),
      needsManualInventoryReview: missingReservation && !skipsInventory
    };
...
async function mutateInventoryState(mutator) {
  ...
  for (let attempt = 0; attempt < MAX_MUTATION_ATTEMPTS; attempt += 1) {
    try {
      ...
      if (!write || write.modified !== false) return output.result;
    } catch (error) {
...
function markItemsSold(state, items = []) {
...
function applySessionUnitAmountOverride(items = [], session = {}, enabled = false) {
  const unitAmount = enabled ? numberOrNull(session.metadata?.promo_unit_amount) : null;
  if (!unitAmount) return items;
...
function publicLineItem(item) {
  return {
    ...
    baseUnitAmount: Number(item.baseUnitAmount || item.unitAmount || 0),
