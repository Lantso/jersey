...
const FORM_MIN_ELAPSED_MS = Number(process.env.LANTSO_FORM_MIN_ELAPSED_MS || 1200);
const FORM_MAX_AGE_MS = Number(process.env.LANTSO_FORM_MAX_AGE_MS || 24 * 60 * 60 * 1000);
const FORM_CLOCK_SKEW_MS = 60 * 1000;
...
export function normalizeClubProfile(body = {}) {
  const guard = validateFormSubmission(body);
...
export function validateFormSubmission(body = {}) {
  const honeypot = clean(body["bot-field"] || body.botField || body.website || body.company, 200);
  if (honeypot) {
    return { ok: false, status: 400, message: "Refresh the page and try again." };
  }

  const now = Date.now();
  const startedAt = Number(body.formStartedAt || body.startedAt || body.loadedAt || 0);
  const submittedAt = Number(body.formSubmittedAt || now);
  if (!Number.isFinite(startedAt) || startedAt <= 0) {
    return { ok: false, status: 400, message: "Refresh the page and try again." };
  }
  if (!Number.isFinite(submittedAt) || submittedAt < startedAt) {
    return { ok: false, status: 400, message: "Refresh the page and try again." };
  }
  if (startedAt > now + FORM_CLOCK_SKEW_MS || submittedAt > now + FORM_CLOCK_SKEW_MS) {
    return { ok: false, status: 400, message: "Refresh the page and try again." };
  }
  if (now - startedAt > FORM_MAX_AGE_MS || submittedAt - startedAt < FORM_MIN_ELAPSED_MS) {
    return { ok: false, status: 400, message: "Refresh the page and try again." };
  }
...
export async function saveClubProfile(record) {
  ...
  if (existing) return { ok: true, key, duplicate: true };
