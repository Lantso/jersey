import crypto from "node:crypto";
import {
  CHECKOUT_ALLOWED_COUNTRIES,
  CURRENCY,
  calculateShipping,
  convertMoney,
  currencyForCountry,
  findProduct
} from "../catalog.mjs";
import {
  attachStripeSessionToReservation,
  releaseInventoryReservation,
  reserveCheckoutInventory
} from "./inventory.mjs";

const STRIPE_CHECKOUT_URL = "https://api.stripe.com/v1/checkout/sessions";
export const CONTENT_SECURITY_POLICY =
  "default-src 'self'; script-src 'self'; style-src 'self' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: https:; connect-src 'self' https://api.stripe.com; frame-src https://checkout.stripe.com; form-action 'self' https://checkout.stripe.com; base-uri 'self'; object-src 'none'";
const RATE_LIMIT_STORE_NAME = "lantso-rate-limits";
const RATE_LIMIT_VERSION = 1;
const MAX_RATE_LIMIT_ATTEMPTS = 4;
let rateLimitStorePromise;
let lastMemoryRateLimitSweep = 0;

export function buildCheckout(body = {}) {
  const requested = Array.isArray(body.items) ? body.items : [];
  const requestedItems = [];
  const inventoryClaims = new Map();

  for (const item of requested) {
    const product = findProduct(item.productId);
    const size = String(item.size || "");
    const quantity = Math.max(1, Math.min(25, Number(item.quantity || 1)));

    if (!product || !product.sizes.includes(size)) {
      return { ok: false, status: 400, message: "Invalid cart item" };
    }

    const inventoryKey = `${product.id}:${size}`;
    const claimed = (inventoryClaims.get(inventoryKey) || 0) + quantity;
    inventoryClaims.set(inventoryKey, claimed);
    if (claimed > product.inventory[size]) {
      return {
        ok: false,
        status: 409,
        message: `${product.name.en} / ${size} only has ${product.inventory[size]} pieces available.`
      };
    }

    const baseUnitAmount = product.price;
    if (baseUnitAmount <= 0) {
      return {
        ok: false,
        status: 400,
        message: "Invalid cart item"
      };
    }

    requestedItems.push({
      productId: product.id,
      sku: product.sku,
      name: product.name.en,
      size,
      quantity,
      baseUnitAmount,
      baseAmount: baseUnitAmount * quantity
    });
  }

  if (!requestedItems.length) return { ok: false, status: 400, message: "Cart is empty" };

  const shippingCountry = String(body.shippingCountry || "FR").toUpperCase();
  if (!CHECKOUT_ALLOWED_COUNTRIES.includes(shippingCountry)) {
    return { ok: false, status: 400, message: "Shipping country is not enabled" };
  }
  const postalCode = String(body.postalCode || "").trim().slice(0, 32);
  const currency = currencyForCountry(shippingCountry);
  const moneyCountry = shippingCountry;
  const items = requestedItems.map((item) => {
    const unitAmount = convertMoney(item.baseUnitAmount, moneyCountry);
    return {
      ...item,
      unitAmount,
      amount: unitAmount * item.quantity
    };
  });

  const baseSubtotal = requestedItems.reduce((sum, item) => sum + item.baseAmount, 0);
  const subtotal = items.reduce((sum, item) => sum + item.amount, 0);
  const quantity = items.reduce((sum, item) => sum + item.quantity, 0);
  const baseShipping = calculateShipping(shippingCountry, baseSubtotal, quantity);
  const shipping = {
    ...baseShipping,
    baseAmount: baseShipping.amount,
    amount: convertMoney(baseShipping.amount, moneyCountry)
  };

  return {
    ok: true,
    items,
    quantity,
    currency,
    baseCurrency: CURRENCY,
    baseSubtotal,
    subtotal,
    shipping,
    shippingCountry,
    postalCode,
    baseTotal: baseSubtotal + baseShipping.amount,
    total: subtotal + shipping.amount,
    orderRef: `LTS-${Date.now().toString(36).toUpperCase()}-${crypto.randomBytes(3).toString("hex").toUpperCase()}`
  };
}

export async function createStripeCheckoutSession(body = {}, options = {}) {
  const checkout = buildCheckout(body);
  if (!checkout.ok) return { ok: false, status: checkout.status || 400, data: { message: checkout.message }, checkout };

  const reservation = await reserveCheckoutInventory(checkout, body);
  if (!reservation.ok) {
    return {
      ok: false,
      status: reservation.status || 409,
      data: { message: reservation.message || "This piece is no longer available." },
      checkout
    };
  }
  checkout.checkoutExpiresAt = reservation.checkoutExpiresAt;
  checkout.holdExpiresAt = reservation.holdExpiresAt;
  checkout.stripeExpiresAtEpoch = reservation.stripeExpiresAtEpoch;

  const secretKey = options.stripeSecretKey || process.env.STRIPE_SECRET_KEY || "";
  if (!secretKey || secretKey.includes("replace_me")) {
    await releaseInventoryReservation(checkout.orderRef, "stripe_not_configured");
    return {
      ok: false,
      status: 503,
      data: {
        message: "Checkout is temporarily unavailable. Contact contact@lantso.com."
      },
      checkout
    };
  }

  const headers = {
    Authorization: `Bearer ${secretKey}`,
    "Content-Type": "application/x-www-form-urlencoded"
  };
  const idempotencyKey = cleanIdempotencyKey(body.idempotencyKey);
  if (idempotencyKey) headers["Idempotency-Key"] = idempotencyKey;
  const apiVersion = options.stripeApiVersion || process.env.STRIPE_API_VERSION || "";
  if (apiVersion) headers["Stripe-Version"] = apiVersion;

  const stripeResponse = await fetch(STRIPE_CHECKOUT_URL, {
    method: "POST",
    headers,
    body: stripeSessionParams(checkout, body, options.siteUrl || process.env.PUBLIC_SITE_URL || "http://127.0.0.1:3000")
  });
  const data = await stripeResponse.json();

  if (!stripeResponse.ok) {
    await releaseInventoryReservation(checkout.orderRef, "stripe_checkout_failed");
    return {
      ok: false,
      status: 502,
      data: { message: data.error?.message || "Stripe checkout failed" },
      checkout
    };
  }

  await attachStripeSessionToReservation(checkout.orderRef, data);
  return { ok: true, status: 200, data: { url: data.url, id: data.id, orderRef: checkout.orderRef }, checkout, stripeSession: data };
}

export function stripeSessionParams(checkout, body = {}, siteUrl) {
  const params = new URLSearchParams();
  const automaticTaxEnabled = process.env.STRIPE_AUTOMATIC_TAX === "true";
  const paymentMethods = selectedPaymentMethods(body.preferredMethod);
  params.set("mode", "payment");
  params.set("success_url", localizedCheckoutUrl(siteUrl, body.language, "/success?session_id={CHECKOUT_SESSION_ID}"));
  params.set("cancel_url", localizedCheckoutUrl(siteUrl, body.language, "/cancel"));
  params.set("locale", checkoutLocale(body.language));
  if (process.env.STRIPE_ADAPTIVE_PRICING === "true") {
    params.set("adaptive_pricing[enabled]", "true");
  }
  if (checkout.stripeExpiresAtEpoch) {
    params.set("expires_at", String(checkout.stripeExpiresAtEpoch));
  }
  params.set("client_reference_id", checkout.orderRef);
  params.set("billing_address_collection", "required");
  params.set("customer_creation", "always");
  params.set("phone_number_collection[enabled]", "true");
  params.set("shipping_address_collection[allowed_countries][]", checkout.shippingCountry);
  params.set("metadata[order_ref]", checkout.orderRef);
  params.set("metadata[shipping_country]", checkout.shippingCountry);
  params.set("metadata[currency]", checkout.currency);
  params.set("metadata[base_currency]", checkout.baseCurrency);
  params.set("metadata[base_subtotal]", String(checkout.baseSubtotal));
  params.set("metadata[base_shipping]", String(checkout.shipping.baseAmount));
  params.set("metadata[base_total]", String(checkout.baseTotal));
  params.set("metadata[stock_action]", "reserve_then_sell");
  if (checkout.postalCode) params.set("metadata[postal_code_hint]", checkout.postalCode);
  params.set("metadata[checkout_expires_at]", checkout.checkoutExpiresAt || "");
  params.set("metadata[stock_hold_expires_at]", checkout.holdExpiresAt || "");
  params.set("metadata[parcel_weight_g]", String(checkout.shipping.weight));
  params.set("metadata[parcel_dimensions_cm]", Object.values(checkout.shipping.dimensions).join("x"));
  params.set("metadata[items]", checkout.items.map((item) => `${item.sku}/${item.size}x${item.quantity}`).join(","));
  params.set("payment_intent_data[description]", `Lantso order ${checkout.orderRef}`);
  params.set("payment_intent_data[metadata][order_ref]", checkout.orderRef);
  params.set("payment_intent_data[metadata][items]", checkout.items.map((item) => `${item.sku}/${item.size}x${item.quantity}`).join(","));
  params.set("automatic_tax[enabled]", automaticTaxEnabled ? "true" : "false");

  paymentMethods.forEach((type) => {
    params.append("payment_method_types[]", type);
  });

  checkout.items.forEach((item, index) => {
    params.set(`line_items[${index}][quantity]`, String(item.quantity));
    params.set(`line_items[${index}][price_data][currency]`, checkout.currency);
    params.set(`line_items[${index}][price_data][unit_amount]`, String(item.unitAmount));
    if (automaticTaxEnabled) {
      params.set(`line_items[${index}][price_data][tax_behavior]`, "inclusive");
    }
    params.set(
      `line_items[${index}][price_data][product_data][name]`,
      `${item.name} / ${item.size}`
    );
    params.set(`line_items[${index}][price_data][product_data][metadata][sku]`, item.sku);
    params.set(`line_items[${index}][price_data][product_data][metadata][size]`, item.size);
  });

  params.set("shipping_options[0][shipping_rate_data][type]", "fixed_amount");
  params.set("shipping_options[0][shipping_rate_data][display_name]", `Tracked home delivery - ${checkout.shipping.zone.label.en}`);
  params.set("shipping_options[0][shipping_rate_data][fixed_amount][amount]", String(checkout.shipping.amount));
  params.set("shipping_options[0][shipping_rate_data][fixed_amount][currency]", checkout.currency);
  if (automaticTaxEnabled) {
    params.set("shipping_options[0][shipping_rate_data][tax_behavior]", "inclusive");
    params.set("shipping_options[0][shipping_rate_data][tax_code]", "txcd_92010001");
  }
  params.set("shipping_options[0][shipping_rate_data][delivery_estimate][minimum][unit]", "business_day");
  params.set("shipping_options[0][shipping_rate_data][delivery_estimate][minimum][value]", "2");
  params.set("shipping_options[0][shipping_rate_data][delivery_estimate][maximum][unit]", "business_day");
  params.set("shipping_options[0][shipping_rate_data][delivery_estimate][maximum][value]", "14");

  return params;
}

function localizedCheckoutUrl(siteUrl, language, path) {
  const cleanSiteUrl = String(siteUrl || "").replace(/\/$/, "");
  const lang = ["fr", "ar"].includes(language) ? `/${language}` : "";
  return `${cleanSiteUrl}${lang}${path}`;
}

function checkoutLocale(language) {
  if (language === "fr") return "fr";
  if (language === "ar") return "ar";
  return "en";
}

function selectedPaymentMethods(preferredMethod) {
  const configured = (process.env.STRIPE_PAYMENT_METHOD_TYPES || "")
    .split(",")
    .map((method) => method.trim())
    .filter(Boolean);
  if (preferredMethod === "paypal" && configured.includes("paypal")) return ["paypal"];
  return configured;
}

export function securityHeaders(contentType = "application/json; charset=utf-8") {
  return {
    "Content-Type": contentType,
    "X-Content-Type-Options": "nosniff",
    "X-Frame-Options": "DENY",
    "Referrer-Policy": "strict-origin-when-cross-origin",
    "Permissions-Policy": "camera=(), microphone=(), geolocation=()",
    "Strict-Transport-Security": "max-age=31536000; includeSubDomains; preload",
    "Content-Security-Policy": CONTENT_SECURITY_POLICY
  };
}

export function corsHeaders(origin, siteUrl = process.env.PUBLIC_SITE_URL || "") {
  if (!origin || !isAllowedOrigin(origin, siteUrl)) return {};
  return {
    "Access-Control-Allow-Origin": origin,
    "Access-Control-Allow-Methods": "GET, POST, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Stripe-Signature",
    "Access-Control-Max-Age": "86400",
    Vary: "Origin"
  };
}

export function isAllowedOrigin(origin, siteUrl = process.env.PUBLIC_SITE_URL || "") {
  // CORS only limits browser callers. Non-browser clients can omit Origin, so public endpoints
  // must rely on validation, rate limiting, and provider-side checks for real protection.
  if (!origin) return true;
  const allowed = new Set(["http://127.0.0.1:3000", "http://localhost:3000"]);
  for (const url of originInputs(siteUrl)) {
    addOriginVariants(allowed, url);
  }
  for (const url of originInputs(process.env.LANTSO_ALLOWED_ORIGINS || "")) {
    addOriginVariants(allowed, url);
  }
  return allowed.has(normalizeOrigin(origin));
}

function originInputs(value) {
  if (Array.isArray(value)) return value.flatMap(originInputs);
  return String(value || "")
    .split(",")
    .map((entry) => entry.trim())
    .filter(Boolean);
}

function addOriginVariants(allowed, value) {
  const normalized = normalizeOrigin(value);
  if (!normalized) return;
  allowed.add(normalized);
  const parsed = new URL(normalized);
  if (parsed.hostname.startsWith("www.")) {
    parsed.hostname = parsed.hostname.slice(4);
    allowed.add(parsed.origin);
    return;
  }
  parsed.hostname = `www.${parsed.hostname}`;
  allowed.add(parsed.origin);
}

function normalizeOrigin(value) {
  try {
    const url = new URL(value);
    if (url.protocol !== "http:" && url.protocol !== "https:") return "";
    return url.origin;
  } catch {
    return "";
  }
}

export async function rateLimit(key, { limit = 60, windowMs = 60_000 } = {}) {
  const normalizedKey = cleanRateLimitKey(key);
  if (!normalizedKey) return { ok: false, remaining: 0, resetAt: Date.now() + windowMs };
  if (process.env.LANTSO_RATE_LIMIT_DRIVER !== "memory") {
    const blobResult = await blobRateLimit(normalizedKey, { limit, windowMs }).catch(() => null);
    if (blobResult) return blobResult;
  }
  return memoryRateLimit(normalizedKey, { limit, windowMs });
}

function memoryRateLimit(key, { limit = 60, windowMs = 60_000 } = {}) {
  const now = Date.now();
  const store = (globalThis.__lantsoRateLimit ||= new Map());
  if (now - lastMemoryRateLimitSweep > windowMs) {
    for (const [entryKey, entry] of store.entries()) {
      if (entry.resetAt <= now) store.delete(entryKey);
    }
    lastMemoryRateLimitSweep = now;
  }
  const current = store.get(key);
  if (!current || current.resetAt <= now) {
    store.set(key, { count: 1, resetAt: now + windowMs });
    return { ok: true, remaining: limit - 1 };
  }
  current.count += 1;
  return { ok: current.count <= limit, remaining: Math.max(0, limit - current.count), resetAt: current.resetAt };
}

async function blobRateLimit(key, { limit = 60, windowMs = 60_000 } = {}) {
  const store = await getRateLimitStore();
  if (!store) return null;
  const safeLimit = Math.max(1, Number(limit || 1));
  const safeWindowMs = Math.max(1000, Number(windowMs || 60_000));
  const blobKey = rateLimitBlobKey(key);

  for (let attempt = 0; attempt < MAX_RATE_LIMIT_ATTEMPTS; attempt += 1) {
    const now = Date.now();
    let current;
    try {
      current = await store.getWithMetadata(blobKey, { type: "json" });
    } catch {
      return null;
    }
    const entry = normalizeRateLimitEntry(current?.data, now);
    const next =
      !entry || entry.resetAt <= now
        ? { version: RATE_LIMIT_VERSION, count: 1, resetAt: now + safeWindowMs }
        : { version: RATE_LIMIT_VERSION, count: entry.count + 1, resetAt: entry.resetAt };
    const result = {
      ok: next.count <= safeLimit,
      remaining: Math.max(0, safeLimit - next.count),
      resetAt: next.resetAt
    };
    const options = {
      metadata: {
        version: String(RATE_LIMIT_VERSION),
        resetAt: String(next.resetAt)
      }
    };
    const etag = current?.etag || current?.metadata?.etag || null;
    if (etag) options.onlyIfMatch = etag;
    else options.onlyIfNew = true;
    let write;
    try {
      write = await store.setJSON(blobKey, next, options);
    } catch {
      return null;
    }
    if (!write || write.modified !== false) return result;
    await backoff(attempt);
  }

  return { ok: false, remaining: 0, resetAt: Date.now() + Math.max(1000, Number(windowMs || 60_000)) };
}

async function getRateLimitStore() {
  if (process.env.LANTSO_STORAGE_DRIVER === "file") return null;
  if (!rateLimitStorePromise) {
    rateLimitStorePromise = import("@netlify/blobs")
      .then(({ getStore }) => getStore(RATE_LIMIT_STORE_NAME))
      .catch(() => null);
  }
  return rateLimitStorePromise;
}

function normalizeRateLimitEntry(entry, now) {
  if (!entry || typeof entry !== "object") return null;
  const count = Number(entry.count || 0);
  const resetAt = Number(entry.resetAt || 0);
  if (!Number.isFinite(count) || !Number.isFinite(resetAt)) return null;
  if (resetAt <= now) return null;
  return { count, resetAt };
}

function rateLimitBlobKey(key) {
  return `v${RATE_LIMIT_VERSION}/${crypto.createHash("sha256").update(key).digest("hex")}.json`;
}

function cleanRateLimitKey(value) {
  return String(value || "").trim().slice(0, 500);
}

function cleanIdempotencyKey(value) {
  const key = String(value || "").trim();
  if (!key || key.length > 180) return "";
  return /^[A-Za-z0-9:_-]+$/.test(key) ? key : "";
}

function backoff(attempt) {
  return new Promise((resolve) => {
    setTimeout(resolve, Math.min(160, 20 * 2 ** attempt));
  });
}
