...
export const SUPPORT_PROMO_CODE = "supercalifragilisticexpialidocious";
const SUPPORT_PROMO_UNIT_AMOUNT = 100;
const SUPPORT_PROMO_MESSAGE =
  "No shirt will be delivered. This is only a test to support the vendor, and stock will not change.";
...
export function buildCheckout(body = {}) {
  ...
  if (promoCode && promoCode !== SUPPORT_PROMO_CODE) {
    return { ok: false, status: 400, message: "Promo code is not active." };
  }
  const isSupportTest = promoCode === SUPPORT_PROMO_CODE;
  ...
  for (const item of requested) {
    ...
    const baseUnitAmount = isSupportTest ? SUPPORT_PROMO_UNIT_AMOUNT : product.price;
    if (baseUnitAmount <= 0) {
      return {
        ok: false,
        status: 400,
  ...
  const currency = isSupportTest ? CURRENCY : currencyForCountry(shippingCountry);
  const moneyCountry = isSupportTest ? "FR" : shippingCountry;
  ...
  const baseShipping = isSupportTest ? supportTestShipping() : calculateShipping(shippingCountry, baseSubtotal, quantity);
  const shipping = {
    ...baseShipping,
  ...
  return {
    ...
    shippingCountry,
    postalCode,
    promoCode: isSupportTest ? SUPPORT_PROMO_CODE : "",
    isSupportTest,
    supportMessage: isSupportTest ? SUPPORT_PROMO_MESSAGE : "",
...
export async function createStripeCheckoutSession(body = {}, options = {}) {
  ...
  const managesInventory = !checkout.isSupportTest;
  if (managesInventory) {
  ...
  if (!secretKey || secretKey.includes("replace_me")) {
    if (managesInventory) await releaseInventoryReservation(checkout.orderRef, "stripe_not_configured");
    return {
      ok: false,
      status: 503,
      data: {
  ...
  if (!stripeResponse.ok) {
    if (managesInventory) await releaseInventoryReservation(checkout.orderRef, "stripe_checkout_failed");
    return {
      ok: false,
      status: 502,
  ...
  if (managesInventory) await attachStripeSessionToReservation(checkout.orderRef, data);
...
export function stripeSessionParams(checkout, body = {}, siteUrl) {
  ...
  if (!paymentMethods.length && !checkout.isSupportTest) {
  ...
  if (!checkout.isSupportTest) {
    params.set("shipping_address_collection[allowed_countries][]", checkout.shippingCountry);
  } else {
    params.set("custom_text[submit][message]", checkout.supportMessage);
  }
  ...
  params.set("metadata[promo_code]", checkout.promoCode || "");
  params.set("metadata[stock_action]", checkout.isSupportTest ? "none" : "reserve_then_sell");
  params.set("metadata[support_test]", checkout.isSupportTest ? "true" : "false");
  params.set("metadata[promo_unit_amount]", checkout.isSupportTest ? String(SUPPORT_PROMO_UNIT_AMOUNT) : "");
  if (checkout.supportMessage) params.set("metadata[order_note]", checkout.supportMessage);
  ...
  checkout.items.forEach((item, index) => {
    ...
    params.set(
      ...
      checkout.isSupportTest ? `Test support - ${item.name} / ${item.size}` : `${item.name} / ${item.size}`
    );
    if (checkout.supportMessage) {
      params.set(`line_items[${index}][price_data][product_data][description]`, checkout.supportMessage);
    }
